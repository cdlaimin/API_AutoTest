...
docker exec -i ${names[0]} pytest -m "$5" --show-capture=no -d --job_name="$1" --build_number="$2" --target="$3" --send_wechat="$6" --wechat_token="$7" --tx socket=${names[1]}:8888 --rsyncdir tests/ tests/ --alluredir=allure-results --clean-alluredir --reruns=1 || true
