def verify(expect, response):
    ...
    if expect:
        if not isinstance(response, dict):
            ...
            if isinstance(expect, dict):
                if expect.get('status_code'):
                    ...
                    assert code == real_code, f'预期状态码：{code} 和实际状态码：{real_code}不匹配'
                else:
                    assert real_code in (200, 201), f'预期状态码：(200, 201) 和实际状态码：{real_code}不匹配'
        ...
        # 没有预期值时要求必须传入响应对象
        assert response.status_code in (200, 201), f'预期状态码：(200, 201) 和实际状态码：{response.status_code}不匹配'
